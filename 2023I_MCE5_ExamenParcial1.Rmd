...
Hipotesis nula (Ho): La facilitación no tiene un efecto significativo en la diversidad de especies en etapas juveniles ni en la reducción de las disparidades de rendimiento entre vecinos de diferentes especies en etapas maduras.

Hipotesis alternativa (Ha): La facilitación tiene un efecto positivo en la diversidad de especies en etapas juveniles, evitando la extinción de especies localmente raras, y reduce las disparidades de rendimiento entre vecinos de diferentes especies en etapas maduras.
...
``````{r crd_plantas}
library(agricolae)
sitios <-c("Balsayacu", "Muyuna", "Alto Tena", "Parque la isla")
r <- 3
seed = 100
...
Se hizo un muestreo de tres sitios en donde se realizo una  búsqueda de especies
raras, en la cual se muestrea la abundancia de las especies, temperatura, sustrato y densidad. 


## Variables
¿Cuáles son sus variables (dependiente(s) e independiente(s))? Explique cada una de las variables

Variable dependientes: Abundancia de especies, temperatura y las Variables independiente:densidad y sustrato. Estas variables son medidas para evaluar el impacto de las variables en la planta. 
